...
struct SPI_CTRL {
	...
	uint8_t TxnByteCtr;			///< Number of to be transmitting bytes.
	uint8_t *PRxData;			///< Pointer to be receiving bytes.
	uint8_t RxnByteCtr;			///< Number of to be receiving bytes.
...
#endif
{
  switch(__even_in_range(UCA0IV,0x04))
  {
    ...
    case 2:
    	if((spi.operate == RECEIVE || spi.operate == TRANSFER) && spi.RxnByteCtr > 0) {
    		*spi.PRxData = UCA0RXBUF;
    		spi.PRxData++;
    	}
    	else {
    ...
    case 4:
    	...
    	if((spi.operate == TRAMSMIT || spi.operate == TRANSFER) && spi.TxnByteCtr > 0) {
    		UCA0TXBUF = *spi.PTxData;             // Transmit characters
    		spi.PTxData++;
    	}
    	else {
